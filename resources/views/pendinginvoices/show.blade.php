...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8">
            <p class="lead"><strong>Job ID:</strong> {{$pendinginvoice->job->id+20100}}</p>
			@if(!empty($pendinginvoice->job->site))
                <p class="lead-md"><strong>Address:</strong> {{$pendinginvoice->job->site->fullMailingAddress()}} </p>
            @else
                <p class="lead-md"><strong>Address:</strong> {{$pendinginvoice->job->client->fullMailingAddress()}}</p>
            @endif
            <hr>
        ...
        <div class="col-md-4">
            <div class="well">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        {!! Html::linkRoute('jobs.show', 'Job Summary', array($pendinginvoice->job_id), array('class'=>'btn btn-default btn-block btn-margin') ) !!}
                    </div>
                    @set('status', count($pendinginvoice->job->pendinginvoices)>0 ? '' : 'disabled')
                    <div class="col-sm-12">
                        {!! Html::linkRoute('invoices.show', 'Invoice Summary', array($pendinginvoice->job->id), array('class'=>'btn btn-default btn-block btn-margin '.$status))!!}
                    </div>
                </div>
